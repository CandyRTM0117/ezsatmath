...
import { Search, X, Check, CircleX, CheckCircle2, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-8">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 rounded-xl text-sm transition-all focus:outline-none min-h-[44px]"
            style={{ background: 'var(--input-bg)', border: '1px solid var(--input-border)', color: 'var(--input-color)' }}
            onFocus={e => (e.target.style.borderColor = 'rgba(59,130,246,0.5)')}
            onBlur={e => (e.target.style.borderColor = 'var(--input-border)')}
          />
        </div>
        <Dropdown value={filterCategory} onChange={handleFilterChange(setFilterCategory)}
          options={[{ value: 'all', label: 'All categories' }, ...CATEGORIES.map(c => ({ value: c, label: c }))]} />
        <Dropdown value={filterDiff} onChange={handleFilterChange(setFilterDiff)}
          options={[
          ...
          >
            <ProblemErrorBoundary>
              ...
              >
                ...
                >
                  <button onClick={closeProblem} className="flex items-center gap-1.5 text-sm font-semibold text-slate-300 hover:text-white transition-colors">
                ...
                {/* Desktop header */}
                <div className="hidden md:flex items-center gap-2 px-4 py-2.5 flex-shrink-0" style={{ borderBottom: '1px solid var(--card-border)' }}>
                  ...
                  <button onClick={closeProblem} className="ml-auto p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
                    <X size={16} />
                  </button>
                </div>
                ...
                <div className="flex flex-col md:flex-row flex-1 overflow-hidden min-h-0 relative">
                  {imgLoading && (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4" style={{ background: 'var(--app-bg-alt)' }}>
                  ...
                  <div className="h-[52%] overflow-y-auto md:h-full md:w-[62%] md:flex-none border-b md:border-b-0 md:border-r" style={{ borderColor: 'var(--card-border)' }}>
                    {selected.image_url && !imgError ? (
                      ...
                      <div className="m-6 rounded-xl flex items-center justify-center text-sm"
